...
|-------|---------|----------------|
| **1.4.4** | **2026-05-17** | **Rilis v1.0.3 — `package.json#version` 1.0.2 → 1.0.3, `src/lib/version.ts#APP_VERSION` 1.0.2 → 1.0.3, dan entry baru di namespace `changelog` (`messages/{en,id}.json`) untuk halaman `/settings/whats-new`. Tagline EN "Cleaner Words, Sharper Edges" / ID "Kata Lebih Bersih, Tampilan Lebih Rapi". Tambah `CHANGELOG.md` di root mengikuti format Keep a Changelog v1.1.0 dengan section Added/Changed/Fixed/Performance/Security/Tests & Docs. Ringkasan rilis: DonutChart Recharts v3 fix, AppHeader 3-column grid centering fix, Loan module terminology rename, transaction list windowing untuk 20k+ entries, security hardening (Next 16.2.6 + CSP/HSTS/X-Frame-Options + dep overrides), in-app FAQ + search, Serwist Turbopack migration, lazy product tour, de-AI microcopy audit, no-comment policy, 100% unit test coverage. Known Issues yang ✅ FIXED 2026-05-17: DonutChart Recharts v3, AppHeader centering, Loan terminology rename, transaction history windowing, security headers.** |
| **1.4.3** | **2026-05-17** | **Copy & microcopy audit — de-AI-fikasi penuh `messages/id.json` dan `messages/en.json`. ~40 string di-rewrite: hapus boilerplate "This action cannot be undone"/"Tindakan ini tidak bisa dibatalkan", passive voice berlebihan, filler marketing (rock-steady, right at home, seamlessly, effortlessly), generic placeholders ("Enter the name" → "e.g. BCA, Cash, GoPay"), dan repetitive no-data descriptions. Tour step `ln2` diperbaiki agar label "Memberi"/"Menerima" sinkron dengan UI. Welcome screen diganti dari "Welcome to PFinTrack!"/"Selamat datang di PFinTrack!" jadi "Hey, welcome!"/"Halo, PFinTrack!". Tambah §16 Konvensi Copy & Microcopy di spec ini. Lint clean, tsc clean, 158 unit tests pass.** |
| **1.4.2** | **2026-05-17** | **Fix off-center title di `AppHeader` (`src/components/shared/AppHeader.tsx`). Sebelumnya: parent flex dengan slot kiri `w-10` (40px) dan slot kanan `min-w-[44px]`, asimetris 4px — title bergeser ke kanan saat back hadir & actions kosong, dan title `max-w-[200px]` membatasi panjang tetapi tidak menjamin centering. Sekarang: layout diubah menjadi **3-column CSS grid** `grid-cols-[44px_1fr_44px]` — slot kiri & kanan **sama lebar 44px** (= `--tap-target-min`), title di tengah grid sehingga **selalu pas di tengah viewport** terlepas dari kombinasi back/actions. Title pakai `truncate max-w-full` di dalam container `min-w-0` agar ellipsis bekerja & tidak overlap dengan slot kiri/kanan. Tombol back diberi `type="button"` agar lulus aturan kode (button semantic). Lint clean, `tsc --noEmit` clean, 158 unit tests pass. Spec global §3.1 di-update.** |
| **1.4.1** | **2026-05-16** | **Perbaikan realisme **Performance Seed** (`src/lib/perf-seed.ts`). (1) **No-negative-balance invariant**: dompet TIDAK PERNAH minus di sepanjang 5 tahun timeline. Mekanisme: setelah generation, ada chronological fix-up pass yang sort semua events (transactions + loan_entries) berdasarkan `transaction_date + transaction_time`, replay urut waktu, dan untuk setiap expense/transfer yang akan men-trigger negatif: (a) auto-insert "Top Up <wallet>" transfer dari BCA sebelum transaksi tersebut, atau (b) skip transaksi (counted ke `skippedForBalance`) kalau BCA sendiri tidak cukup. Helper `tryTopUpForSpend(walletId, amount, date, time)` insert top-up dengan amount = deficit + buffer 300k, di-timestamp 1 menit sebelum expense. (2) **Density 10-20 tx/hari**: routine harian diperluas dari 4 entry jadi 14 step (sarapan / kopi pagi / ojek pergi / mid-morning snack / air mineral / lunch / es teh siang / snack sore / ojek pulang / convenience store / dinner / snack malam / parkir / pulsa). Probability boost di setiap step. Tambah katalog `BREAKFAST`, `SNACK_DRINK`, `CONVENIENCE` (Indomaret/Alfamart/Lawson/FamilyMart/Circle K), `PULSA_DATA`. Total: median 11 tx/hari (p25=9, p75=12, max=21, avg=10.3), 5-year dataset jadi **~18.700 transactions** (sebelumnya ~8.200). (3) Opening BCA naik 12.5jt → 50jt; salary base 12.5jt → 15jt (kenaikan masih 8%/tahun). Monthly DCA Reksadana 1.5jt → 2jt, Tabungan Emas 500k → 700k, top-up Jago 3jt → 3.5jt, top-up GoPay bulanan baru 1.5jt. (4) Output file size 3.5MB → 7.5MB (uncompressed JSON). Generation time tetap <150 ms. Verifikasi: chronological replay di Node menunjukkan min balance ever per wallet = 0 untuk semua sub-wallet (= titik tepat sebelum opening balance injection), BCA min ~24k (positif).** |
| **1.4.0** | **2026-05-16** | **Tambah dev-only **Performance Seed** untuk uji performa dengan dataset realistis 5 tahun ke belakang — CLI-only, tidak menyentuh production code path. (1) `src/lib/perf-seed.ts` — pure generator function `generatePerfSeedData({ anonId, years?, seed?, today? })` yang return `BackupData` (format yang sama dengan fitur Export/Import yang sudah ada). Tidak ada IDB/DOM/window dependency → bisa dijalankan di Node. (2) `scripts/generate-perf-seed.ts` — CLI yang import generator, dump JSON ke file, dengan flag `--out / --anon-id / --years / --seed / --today`. Pakai `tsx` (added as devDep) untuk eksekusi langsung TypeScript. NPM script baru: `npm run seed:perf`. (3) Output: file `pfintrack-perf-seed.json` (~3.5 MB, sudah di-`.gitignore`) berisi 6 wallets, ~8.200 transactions, ~20 `wallet_balance_history`, 5 loan counterparties (Budi/Sinta/Andi/Dewi/Pak RT) dengan 21 entries, 3 custom report definitions. Pola realistis: gaji bulanan tanggal 25 + kenaikan 8%/tahun, THR Juni 15 + spending Lebaran, bonus akhir tahun (Des 20), freelance acak ~7-10x/tahun, daily food 2-4x/hari dari 35+ merchant Indonesia (Nasi Padang/Hokben/Kopi Kenangan/Mixue/dll), groceries Sabtu, monthly bills tanggal 1-5 (listrik PLN/IndiHome/BPJS/KPR/PAM/streaming subs), transport harian + bensin 10-harian, weekend entertainment, 3 liburan/tahun (Bali/Yogya/Bandung/Lombok/Singapore/Malang), 1-2 big purchase/tahun (laptop/HP/TV/dll), 2-4 balance correction/tahun, inflasi ~6% YoY. PRNG mulberry32 seed `0xc0ffee` → deterministik. Generation time: **~90 ms** di MacBook M-series untuk 5-year dataset. (4) **Cara pakai**: `npm run seed:perf -- --anon-id $(localStorage.pfintrack_anon_id)` → buka app → Settings → Restore Backup → pilih `pfintrack-perf-seed.json`. AppProviders.tsx **tidak** dipolusi — generator hanya digunakan dari CLI/Node. Tidak menyentuh `injectDemoData` yang existing.** |
| **1.3.2** | **2026-05-16** | **Coverage report fix untuk `src/lib/storage/anon-id.ts`. Setelah v1.3.1, file ini masih tampil sebagai `0 \| 0 \| 0 \| 0` di reporter text V8 walaupun ter-import oleh test, karena V8 coverage provider menghitung modul yang isinya **murni re-export** (`export { a, b } from "..."`) sebagai 0 executable statements (no bytecode → 0/0 → ditampilkan 0%). Aggregate summary tidak terpengaruh (`All files` tetap 100%) tapi reporter per-file misleading. Fix: tambah `src/lib/storage/anon-id.ts` ke `coverage.exclude` di `vitest.config.ts` (ikut pola `src/lib/storage/*.ts` repos lain yang juga di-exclude karena tidak unit-testable / pure shim). Logic asli tetap 100% covered via `src/lib/bootstrap/anon-id.ts`. File test `src/lib/storage/anon-id.test.ts` tetap dipertahankan sebagai integration smoke test (verifikasi re-export referential identity). Hasil: 158 tests passing, coverage `All files` 100% Stmts/Branch/Funcs/Lines, dan reporter per-file tidak lagi menampilkan baris 0%.** |
| **1.3.1** | **2026-05-16** | **Tutup sisa gap unit-test coverage agar `All files` kembali 100% di semua metric (stmts/branch/funcs/lines). (1) Tambah test baru di `src/lib/report/calculations.test.ts` (suite `calculateMonthlySummary` → "startBalance accumulates balance corrections before period") untuk meng-cover baris 304 (`prevCorrections` reducer di `calculateMonthlySummary`): skenario dengan `WalletBalanceHistory` aktif `corrected_at < start` (dijumlahkan ke `startBalance`), entry di dalam periode (dihitung sebagai `balanceCorrection`), serta entry inactive sebelum periode (di-skip). (2) Tambah file baru `src/lib/storage/anon-id.test.ts` (sebelumnya 0% di semua metric) — verifikasi bahwa `getOrCreateAnonId` dan `readAnonId` di-re-export apa adanya dari `@/lib/bootstrap/anon-id` (referential identity via `expect.toBe`), plus smoke test perilaku create/persist/read identik dengan implementasi bootstrap. Hasil akhir: 10 test files, **158 tests passing** (sebelumnya 153), coverage `All files` = 100% Stmts (250/250) · 100% Branch (132/132) · 100% Funcs (77/77) · 100% Lines (217/217). `npm run lint` zero warning.** |
...
| **`/settings/faq`** | **Settings** | **In-app FAQ. 14 pertanyaan bilingual dalam 3 kategori (Privasi & Data / Fitur & Cara Kerja / Teknis & Operasional). Multi-open accordion native `<details>`/`<summary>` + search realtime client-side. Diakses dari row "FAQ" di section Help. **Spec lengkap: `tech-spec-feature-faq.md`.** |
...
| Judul | Rata tengah viewport. Bold. Bisa dengan subtitle (mis. di Loan Detail menampilkan "Not paid off"). |
...
| Layout | **3-column CSS grid** `grid-cols-[44px_1fr_44px]` — kolom kiri (back/spacer 44px) · kolom tengah (title+subtitle, truncate) · kolom kanan (actions/spacer 44px). Lebar kiri & kanan **selalu sama** sehingga title benar-benar di tengah viewport, terlepas dari ada/tidaknya tombol back atau action. Title menggunakan `truncate max-w-full`, container tengah `min-w-0` agar ellipsis bekerja saat label panjang dan tidak overlap dengan side slots. |
...
> Konvensi Semantic HTML tersedia di **§12** dokumen ini. Security posture & hardening di **§14**. Konvensi copy & microcopy di **§16**.
...
## 16. Konvensi Copy & Microcopy (De-AI-fikasi)

**Berlaku mulai:** 2026-05-17

Semua teks UI — button, label, empty state, dialog, error, onboarding — wajib mengikuti prinsip berikut:

1. **Tone santai, bukan corporate.** Pakai "kamu" (ID) dan "you" + contractions (EN). Hindari "Anda", "para pengguna", "telah berhasil disimpan oleh sistem".
2. **Kalimat pendek menang.** Kalau bisa 5 kata, jangan pakai 12. Pecah klausa panjang jadi dua kalimat.
3. **Hapus filler boilerplate.** Frasa seperti "This action cannot be undone", "Tindakan ini tidak bisa dibatalkan", "Please select a", dan "Enter the" diganti dengan kalimat yang langsung ke inti.
4. **Hindari marketing hyperbole.** Kata seperti "seamlessly", "effortlessly", "rock-steady", "right at home" tidak boleh masuk copy UI. Kalau ada, ganti dengan deskripsi konkret.
5. **Em-dash berlebihan adalah penanda AI.** Satu atau dua per halaman masih wajar. Em-dash di setiap kalimat panjang: refactor ke titik atau koma.

**Glosarium copy wajib:** lihat `CLAUDE.md` (system prompt PFinTrack Content Translator) — kolom "santai/UI" untuk button/label/empty state, kolom "formal" untuk error serius dan dokumentasi.

**Audit terakhir:** 2026-05-17 — kedua file `messages/id.json` dan `messages/en.json` sudah di-audit dan di-rewrite penuh mengikuti prinsip di atas.

---

*— End of Technical Specification: Global Architecture (v1.4.2) —*
