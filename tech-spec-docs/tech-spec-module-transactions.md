...
| **Performance Search** | Untuk Fase 1, pencarian dilakukan in-memory (filter array). Asumsi jumlah transaksi user di bawah 10.000 record — performa filter masih sangat baik. Lihat catatan **History Pagination / Windowing** di bawah untuk batasan rendering. |
| **History Pagination / Windowing** ✅ FIXED 2026-05-16 | Sebelumnya `/transactions/history` me-render seluruh hasil `filtered` sekaligus. Pada dataset besar (~8.200 transaksi, mis. setelah Restore Backup dari perf-seed) halaman freeze 5–20 detik atau crash tab karena ribuan `<TransactionItem>` di-mount sinkron (masing-masing memanggil `useTranslations`, `useLocale`, `useRouter`, `useLongPress`, `wallets.find`). Sejak 2026-05-16 halaman menggunakan **windowing sederhana berbasis IntersectionObserver**: render awal hanya `PAGE_SIZE = 50` baris pertama, dan menambah `+50` setiap kali sentinel di bawah list masuk viewport (rootMargin 400px untuk pre-load saat scroll cepat). `visibleCount` di-reset ke `PAGE_SIZE` setiap kali filter type / wallet / search / sort / date range berubah. Counter `tc("items", { count })` tetap menampilkan total hasil filter (bukan jumlah yang terlihat). Initial render dataset 8.200 tx turun dari ~5–20 detik / hang menjadi **< 1 detik**. Catatan kapasitas: pendekatan ini cukup untuk dataset hingga ratusan ribu transaksi karena `filtered.slice(0, visibleCount)` dan IntersectionObserver bekerja O(visible). Filter/sort tetap O(N) — masih layak untuk N < 100k. |
