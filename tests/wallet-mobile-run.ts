...
async function runTests() {
  ...
  for (const vp of VIEWPORTS) {
    for (const route of ROUTES) {
      ...
      if (headerCount > 1) {
        issues.push(
          `${tag} DUPLICATE HEADER — found ${headerCount} <header> elements`
        );
      } else if (headerCount === 0) {
        issues.push(`${tag} No header found`);
      } else {
        passes.push(`${tag} Single header (count=${headerCount})`);
      }

      // ── 3. Header visible + position ─────────────────────────────
      ...
      if (!headerVisible) {
        ...
        if (hBox) {
          ...
          if (hBox.height < 44 || hBox.height > 90) {
            issues.push(
              `${tag} Header height unexpected: ${hBox.height}px (expected 44–90)`
            );
          } else {
            passes.push(`${tag} Header height OK: ${hBox.height}px`);
          }
          console.log(
            `  Header: y=${hBox.y}, h=${hBox.height}, visible=${headerVisible}`
          );
        }
      }
      ...
      // BottomNav is always rendered (in root layout), even on /add
      if (!navVisible) {
        ...
        if (navBox) {
          ...
          if (navBottom < vp.height - 20) {
            issues.push(
              `${tag} BottomNav not at bottom — bottom=${navBottom}, viewport=${vp.height}`
            );
          } else {
            passes.push(`${tag} BottomNav anchored at bottom`);
          }
          ...
          console.log(
            `  BottomNav: y=${navBox.y}, h=${navBox.height}, bottom=${navBottom}, tabs=${navCount}`
          );
        }
      }
      ...
      if (route.path === "/wallet") {
        ...
        if (!fabVisible) {
          ...
          if (fabBox && navBox) {
            ...
            if (fabBottom > navTop) {
              issues.push(
                `${tag} FAB overlaps BottomNav — FAB bottom=${fabBottom}, nav top=${navTop}`
              );
            } else {
              passes.push(
                `${tag} FAB above BottomNav (FAB bottom=${fabBottom}, nav top=${navTop})`
              );
            }
            const fabRight = fabBox.x + fabBox.width;
            if (fabRight < vp.width - 20) {
              issues.push(
                `${tag} FAB not in bottom-right — right edge=${fabRight}, viewport=${vp.width}`
              );
            } else {
            ...
            if (fabBox.width < 44 || fabBox.height < 44) {
              issues.push(
                `${tag} FAB too small: ${fabBox.width}×${fabBox.height}px`
              );
            } else {
              passes.push(`${tag} FAB size OK: ${fabBox.width}×${fabBox.height}px`);
            }
            console.log(
              `  FAB: x=${fabBox.x}, y=${fabBox.y}, w=${fabBox.width}, h=${fabBox.height}, bottom=${fabBottom}`
            );
          }
        }
      }
      ...
      const interactiveSelectors = ["button", "a[href]"];
