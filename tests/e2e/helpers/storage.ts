...
export async function seedWallets(page: Page, wallets: SeedWallet[]) {
  ...
  await idbPut(page, "wallets", wallets.map((w, i) => ({
    id: w.id, anon_id: TEST_ANON_ID, name: w.name,
    wallet_type: w.wallet_type ?? "bank", balance: w.balance,
    currency: "IDR", sort_order: i, is_active: true, created_at: ts, updated_at: ts,
  })));
}
...
export async function seedTransactions(page: Page, transactions: SeedTransaction[]) {
  ...
  await idbPut(page, "transactions", transactions.map((t) => ({
    id: t.id, anon_id: TEST_ANON_ID, type: t.type,
    wallet_id: t.wallet_id, destination_wallet_id: t.destination_wallet_id ?? null,
    amount: t.amount, title: t.title ?? null, category: t.category ?? null,
    description: null, transaction_date: t.transaction_date, transaction_time: "10:00",
...
export async function seedCounterparties(page: Page, counterparties: SeedCounterparty[]) {
  ...
  await idbPut(page, "loan_counterparties", counterparties.map((cp) => ({
    id: cp.id, anon_id: TEST_ANON_ID, name: cp.name,
    is_active: true, created_at: ts, updated_at: ts,
  })));
}
...
export async function seedLoanEntries(page: Page, entries: SeedLoanEntry[]) {
  ...
  await idbPut(page, "loan_entries", entries.map((e) => ({
    id: e.id, anon_id: TEST_ANON_ID, counterparty_id: e.counterparty_id,
    type: e.type, wallet_id: null, amount: e.amount,
    description: e.description ?? null, entry_date: e.entry_date,
    is_paid_off: e.is_paid_off ?? false, is_active: true,
